...
function getTotalProductWeight(productLineItemsModel) {
    ...
    productLineItemsModel.items.forEach(function (item) {
        var itemProductWeight = 0;
        if(item.quantity > 1) {
            itemProductWeight = item.quantity * item.productWeight;
        } else {
            itemProductWeight += item.productWeight;
        }
        totalProductWeight += itemProductWeight;
    });
...
function CartModel(basket) {
    ...
    if (basket !== null) {
        ...
        this.totalProductWeight = getTotalProductWeight(productLineItemsModel);
